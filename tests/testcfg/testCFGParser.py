...
class TestCFGCase(unittest.TestCase):
    ...
    def test_get_station_name(self):
        self.assertEqual('xtz', self.parser.station_name)

    def test_get_channel_num(self):
        self.assertEqual(self.parser.channel_total_num, 144)

    def test_get_sample_num(self):
        self.assertEqual(self.parser.sample_total_num, 3077)
    ...
    def test_get_zero_time_from_cfg(self):
        self.assertEqual(199936, self.parser.fault_time.zero_time)
    ...
    def test_get_analog_num(self):
        self.assertEqual(48, self.parser.analog_channel_num)

    def test_get_channel_info(self):
        self.assertEqual("220kV母线II_Ub", self.parser.get_channel_info(6, 'chid'))
        self.assertEqual("220kV母线II_U", self.parser.get_channel_info(6, 'ccbm'))
        self.assertEqual("220kVfx2x_211开关_保护一_C相跳闸", self.parser.get_channel_info(6, 'chid', _type='digital'))

    def test_is_analog_usage(self):
        self.assertEqual(True, self.parser.analog_channels[6].ratio == 1)
        self.assertEqual(False, self.parser.analog_channels[37].ratio == 1)

    def test_get_analog_ratio(self):
        self.assertEqual(2200.0, self.parser.analog_channels[6])
        self.assertEqual(320.0, self.parser.analog_channels[25])
        self.assertEqual(1.0, self.parser.analog_channels[37])

    def test_is_primary_analog(self):
        self.assertEqual(False, self.parser.analog_channels[6].primary)

    def test_get_digital_num(self):
        self.assertEqual(96, self.parser.digital_channel_num)
