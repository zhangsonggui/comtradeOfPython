...
class TransformerWinding(BaseModel):
    ...
    vrtg: float = Field(default=0.0, description="额定电压")
    artg: float = Field(default=0.0, description="一次额定电流")
    ...
    def __str__(self):
        xml = f"<scl:TransformerWinding location={self.location.get_code()} srcRef={self.reference} VRtg={self.vrtg} ARtg={self.artg} bran_num={self.bran_num} bus_ID={self.bus_id} wG="">"
...
class Transformer(PrimaryEquipments):
    pwrRtg: float = Field(default=0.0, description="变压器额定功率")
    ...
    def __str__(self):
        xml = f"<scl:Transformer idx={self.idx} trm_name={self.name} srcRef={self.reference} pwrRtg={self.pwrRtg} transformer_uuid="">"
